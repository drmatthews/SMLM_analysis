...
class Estimate:
    ...
    def __init__(self, samples, threshold_method, threshold):
        ...
        Args:
            samples (int): Size of kernel used in filtering.
            threshold_method (str): Threshold method
                (either `std` or `manual`).
            threshold (float, optional): Intensity threshold for
                position estimation - to be used if threshold_method
                is `manual`.            
    ...
    def run(self, image):
        ...
        Args:
            image (numpy array): A numpy array representation of an image
                with float64 dtype.

        Returns:
            coords (numpy array): An array of coordinates of bright spots
                in the input image
        """
        # compound filter
...
def save_positions(path, results):
    """Save estimated positions to csv.None

    Args:
        path (str): The path to the csv file.None
        results (list): A list of numpy arrays of esitimated coordinates.
    """
...
def process(movie, start, stop, samples,
    ...
    Args:
        movie (pims instance): A multi-frame image sequence.
        samples (int): The size in pixels of the kernel in the
            compound filter.
        threshold_method (str): Either `std` or `manual` [if `std` the
            threshold is etimated from the filtered image].
        threshold (float): Grey level above which bright spots are retained.
        notebook (bool): True if the script is run from a Jupyter notebook.

    Returns:
        results (list): A list of numpy array representation of the
            coordinates of the bright spots found in each image of the
            movie.
    """
    pbar = tqdm.tqdm
    if notebook:
...
def analyse_movie(movie_path, parameters_path, notebook=True):
    ...
    Args:
        movie_path (str): Path to movie in ND2 format to be processed.
        parameters_path (str): Path to file in yaml format holding the
...
def test_frame(movie_path, parameters_path, frame_no):
    ...
    Args:
        ...
        parameters_path (str): Path to file in yaml format holding the
            paramters for position estimation.
        frame_no (int): The index of the frame to process.
    """    
    if os.path.exists(movie_path) and os.path.exists(parameters_path):
        if movie_path.endswith('nd2'):
            movie = pims.open(movie_path)
